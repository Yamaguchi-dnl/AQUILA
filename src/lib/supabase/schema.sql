...
DROP POLICY IF EXISTS "Admins can manage all content" ON pages;
DROP POLICY IF EXISTS "Admins can manage all content" ON blocks;

-- Policies for 'pages' table
CREATE POLICY "Read access for everyone" ON pages
FOR SELECT USING (true);

CREATE POLICY "Admins can manage all content" ON pages
FOR ALL USING (
...
CREATE POLICY "Admins can manage all content" ON blocks
FOR ALL USING (
  (SELECT is_admin FROM public.users WHERE id = auth.uid()) = true
) WITH CHECK (
  (SELECT is_admin FROM public.users WHERE id = auth.uid()) = true
...
-- Function to set the updated_by and updated_at fields automatically
...
CREATE TRIGGER on_block_change
  ...
  FOR EACH ROW
  EXECUTE PROCEDURE set_update_metadata();