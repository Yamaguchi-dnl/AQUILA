...
export function StatsCard() {
    return (
        <div className="relative z-20">
            <div className="">
                <Card className="p-8 bg-card backdrop-blur-md border shadow-xl">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-y-8 text-center">
                        {stats.map((stat, index) => (
                            <div key={stat.label} className="relative flex flex-col items-center justify-center">
                                {index > 0 && <Separator orientation="vertical" className="absolute left-0 top-1/2 -translate-y-1/2 h-1/2 hidden md:block bg-border" />}
